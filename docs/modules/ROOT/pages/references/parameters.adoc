...
The example restricts the use of the resulting storage class `localblock-storage` to namespaces labelled with `syn.tools/component=rook-ceph`.
  ...
  openshift4_local_storage:
    local_volumes:
      ...
      storagevolumes:
        ...
        # labelled with `syn.tools/component=rook-ceph`.
        restricted_to:
          syn.tools/component:
            values:
              - rook-ceph
